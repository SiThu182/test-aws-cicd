...
export default function ResendPage() {
    ...
    let emailSend = async () => {
      ...
      try {
        res = await axios.post(
          `${process.env.REACT_APP_BACKEND_API}resend-verification-code`,
          {
            email: data.email,
            // name: data.name,
          }
  ...
  return (
    <>
      <ReusableLayout title={"Resend Verification Code"} backPath="/logIn">
        <>
          <Typography variant="body2" sx={{ mb: 2 }} color="text.secondary">
            Please type your account that is already registered .We will mail
            verification code to that account .
          </Typography>
