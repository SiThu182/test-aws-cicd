...
function RecordingCourseForm() {
  ...
  let saveType = async (data) => {
    ...
    formData.append("rec_course_id", state.plan.id);
  ...
  useEffect(() => {
    if (sendRegister) {
      ...
      if (subscriptionFrontendRegisterStatus === "succeeded") {
        console.log(subscriptionFrontendRegister.payment_method,"subscriptionFrontendRegister")
